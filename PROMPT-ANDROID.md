...
Salin seluruh blok di bawah dan kirim ke agent. Ditulis dalam bahasa
Inggris karena agent coding umumnya lebih akurat begitu.

**Sebelum mengirim**, ganti dua hal:
- `https://192.168.1.5:8000` → alamat laptop kalian (`make_cert.py`
  mencetaknya)
- `applicationId` kalau mau nama paket sendiri

---

```
Build an Android app in Kotlin called "Q-Shield Scanner". It is a
reference client for an existing QRIS anti-fraud API. The API already
exists and is fully specified below — do not invent endpoints or
fields.
...
2. RESULT    — a card showing the verdict. Colours:
                 ...
                 step_up      orange
                 cooling_off  red
               Show every string from `reasons` as a bulleted list,
               largest weight first (the API already returns them in
               ...
               ID yang tercetak di stiker" and a text field where the
               user can type what is printed, plus a "Periksa" button
               that re-submits with that value.
3. ERROR     — network or permission failure, with a retry button.

Display language is Indonesian. Keep the copy exactly as given below.
...
4. Scan WiFi. Use WifiManager.scanResults. Since Android 9 scanning is
   throttled to roughly 4 calls per 2 minutes, so cache the last result
   for 60 seconds and reuse it rather than calling every scan.
   For each result, hash the BSSID like this and send ONLY the hash:
   ...
   NEVER send the raw BSSID or SSID anywhere. Take at most 32 APs,
   sorted by signal level descending.
5. Detect root with a simple heuristic: check for the existence of
...
6. Device id: generate a random UUID on first launch, store it in
   DataStore, reuse it forever. It must match ^[A-Za-z0-9_-]{8,64}$.
   Never use ANDROID_ID, IMEI, or any hardware identifier.

## The API

Base URL is configurable in a settings screen; default
...
Request body — every field named here is real, nothing else is accepted:
...
- device_integrity, ambient_wifi, printed_label are optional. Omit the
  whole object rather than sending nulls inside it when you have
  nothing. Omitting is not penalised by the server.
- accuracy_m must be the real value from the Location object. Do not
  round it, do not substitute a constant. Values below 1.0 are treated
  by the server as physically impossible and raise the risk score.
- Set "attested" to null. App Attest / Play Integrity is a later step.
...
Map `action` to the UI, not `verdict`. Ignore unknown fields in the
response — new ones get added without warning.

Error codes: 401 invalid API key, 413 body too large, 422 validation
failed or payload is not valid QRIS, 429 rate limited, 503 server not
configured. Show the Indonesian message for each:
...
## Privacy constraints — these are not negotiable
...
- Do not log the QRIS payload or the device UUID to Logcat in release
  builds.
- Do not add analytics, crash reporting, or any third-party SDK.

## What to deliver

Complete, compiling project: Gradle files, manifest with permissions,
...
Add unit tests for: the BSSID hashing function (same input gives same
32-char lowercase hex), the request builder (optional objects omitted
when empty, never sent as nulls inside), and the action-to-colour
mapping.
...
Yang harus kalian periksa sendiri, karena agent tidak bisa:

1. **Hash BSSID benar-benar dikirim, bukan BSSID mentah.** Buka
   Logcat atau proxy, pastikan tidak ada yang berbentuk `aa:bb:cc:...`
2. **accuracy_m nilai asli**, bukan konstanta. Ini sering disederhanakan
   agent, dan server memperlakukan nilai di bawah 1,0 sebagai mustahil.
3. **Aplikasi tetap jalan saat izin WiFi ditolak** — field-nya dihilangkan,
   bukan dikirim kosong.
4. **Tidak ada SDK pihak ketiga** yang diam-diam ditambahkan.

## Cara menguji ke server kalian
...
Lalu pindai `props/01-asli.png` dan `props/02-swap.png` dari aplikasi.
Yang pertama harus `proceed`, yang kedua `cooling_off`.

Untuk melihat sidik jari WiFi benar-benar masuk:
